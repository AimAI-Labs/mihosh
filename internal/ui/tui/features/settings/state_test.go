...
func TestUpdate_NonEditMode_UpDown(t *testing.T) {
	...
	// 已经在 0，继续按 Up 不应越界
	next.Config = cfg
	next.configSvc = configSvc
	next, _ = next.Update(upMsg)
	if next.selectedSetting != 0 {
...
func TestUpdate_LanguageEditMode(t *testing.T) {
	...
	next.Config = cfg
	next.configSvc = configSvc
	nextEnter, _ := next.Update(enterMsg)
	if nextEnter.editMode {
		t.Errorf("expected editMode to be false after Enter")
	}
	if nextEnter.Config.Language != "en-US" {
		t.Errorf("expected saved language to be en-US, got %q", nextEnter.Config.Language)
	}
	...
	sInvalid.Config = cfg
	sInvalid.configSvc = configSvc
	nextInvalid, _ := sInvalid.Update(enterMsg)
...
func TestUpdate_NormalEditMode_CursorAndInput(t *testing.T) {
	...
	next.Config = cfg
	next.configSvc = configSvc
	next, _ = next.Update(backspaceMsg)
	if next.editValue != "http://example.coms" {
...
func TestUpdate_NormalEditMode_Enter_Success(t *testing.T) {
	...
	if next.Config.Timeout != 9999 {
		t.Errorf("expected saved timeout to be 9999, got %d", next.Config.Timeout)
	}
...
func TestThemeSwitchHotSwap(t *testing.T) {
	...
	if next.Config.Theme != "catppuccin" {
		t.Fatalf("expected cfg.Theme=catppuccin, got %q", next.Config.Theme)
	}
}
...
func TestUpdate_SwitchTabByKey(t *testing.T) {
	...
	next.Config = cfg
	next.configSvc = configSvc
	next, _ = next.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("l")})
	if next.activeTab != 0 {
...
func TestHandleMouseLeft_ClickTabBarSwitchesTab(t *testing.T) {
	...
	s.Config = cfg
	s.configSvc = configSvc
	next, _ := s.HandleMouseLeft(mihomoTabX, settingsTabBarContentY, 100, 30)
	if next.activeTab != 1 {
	...
	next.Config = cfg
	next.configSvc = configSvc
	next, _ = next.HandleMouseLeft(mihoshTabX, settingsTabBarContentY, 100, 30)
	if next.activeTab != 0 {
