...
func TestHandleMouseLeft_ClickTabBarSwitchesTab(t *testing.T) {
	...
	next, _ := s.HandleMouseLeft(mihomoTabX, settingsTabBarContentY, cfg, configSvc, nil)
	if next.activeTab != 1 {
	...
	next, _ = next.HandleMouseLeft(mihoshTabX, settingsTabBarContentY, cfg, configSvc, nil)
	if next.activeTab != 0 {
